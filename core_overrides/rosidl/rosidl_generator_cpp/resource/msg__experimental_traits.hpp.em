...
message_namespace = '::'.join(
    list(message.structure.namespaced_type.namespaces) + ['experimental'])
...
# ROS fully-qualified name is the same as the non-experimental counterpart
# to preserve type identity across the wire.
...
@[    if isinstance(member.type, BasicType)]@
    out << "@(member.name): ";
@[      if member.type.typename in EXPERIMENTAL_CHARACTER_TYPES]@
    rosidl_generator_traits::character_value_to_yaml(msg.@(member.name).get(), out);
@[      else]@
    rosidl_generator_traits::value_to_yaml(msg.@(member.name).get(), out);
@[      end if]@
...
@[        if member.type.value_type.typename in EXPERIMENTAL_CHARACTER_TYPES]@
...
inline std::string to_yaml(
  const @(message.structure.namespaced_type.name) & msg, bool use_flow_style = false)
{
...
@[else]@
  return std::forward_as_tuple(
@{forward_args = ['    std::forward<T>(msg).{}'.format(member.name) for member in message.structure.members]}@
@(',\n'.join(forward_args)));
@[end if]@
}
...
for member in message.structure.members:
    ...
    if isinstance(type_, AbstractGenericString):
        ...
        break
    if isinstance(type_, NamespacedType):
        exp_typename = experimental_namespaced_type_name(type_)
        fixed_template_strings.add('has_fixed_size<{}>::value'.format(exp_typename))
else:
...
for member in message.structure.members:
    ...
    if isinstance(type_, NamespacedType):
        exp_typename = experimental_namespaced_type_name(type_)
        bounded_template_strings.add('has_bounded_size<{}>::value'.format(exp_typename))
else:
