...
XCdrAccessor::Iterator::Iterator(XCdrAccessor & accessor, size_t index)
: accessor_(&accessor), index_(index)
{
}

XCdrAccessor XCdrAccessor::Iterator::operator*() const
{
...
XCdrAccessor::AccessorArrowHelper XCdrAccessor::Iterator::operator->()
{
  auto result = accessor_->item(index_);
  if (!result) {
    throw result.error();
  }
  return AccessorArrowHelper(*result);
}

XCdrAccessor::Iterator & XCdrAccessor::Iterator::operator++()
{
  ++index_;
  return *this;
}

XCdrAccessor::Iterator XCdrAccessor::Iterator::operator++(int)  // NOLINT
{
  Iterator temp = *this;
  ++(*this);
  return temp;
...
XCdrAccessor::Iterator & XCdrAccessor::Iterator::operator+=(difference_type n)
{
  index_ += n;
  return *this;
}
...
XCdrAccessor::Iterator operator+(
  const XCdrAccessor::Iterator & it,
  XCdrAccessor::Iterator::difference_type n)
{
  XCdrAccessor::Iterator result = it;
  result += n;
  return result;
}
...
XCdrAccessor::Iterator::difference_type operator-(
  const XCdrAccessor::Iterator & lhs,
  const XCdrAccessor::Iterator & rhs)
{
  return static_cast<XCdrAccessor::Iterator::difference_type>(lhs.index_) -
         static_cast<XCdrAccessor::Iterator::difference_type>(rhs.index_);
}

bool operator==(const XCdrAccessor::Iterator & lhs, const XCdrAccessor::Iterator & rhs)
{
...
// XCdrAccessor implementation

XCdrResult<XCdrAccessor> XCdrAccessor::wrap(
  ...
  const XCdrStructLayout & layout)
{
  ...
  return XCdrAccessor(buffer, layout);
}

XCdrAccessor::XCdrAccessor(
  tcb::span<uint8_t> buffer,
  const XCdrStructLayout & layout)
: buffer_(buffer),
...
XCdrResult<XCdrAccessor> XCdrAccessor::member(std::string_view name_or_path)
{
...
XCdrAccessor XCdrAccessor::operator[](std::string_view name_or_path)
{
...
XCdrResult<XCdrAccessor> XCdrAccessor::item(size_t index)
{
  // Get the struct layout (either from top-level or from polymorphic layout)
  const XCdrStructLayout * struct_layout_ptr = nullptr;
  ...
  if (std::holds_alternative<XCdrPrimitiveArrayLayout>(*layout_)) {
    ...
    return XCdrAccessor(buffer_, &get_primitive_layout(prim_array.element_kind()), elem_offset);
  }
  ...
  if (std::holds_alternative<XCdrArrayLayout>(*layout_)) {
    ...
    return XCdrAccessor(buffer_, &layout_result->get(), elem_offset);
  }
  ...
  if (std::holds_alternative<XCdrPrimitiveSequenceLayout>(*layout_)) {
    ...
    return XCdrAccessor(buffer_, &get_primitive_layout(prim_seq.element_kind()), elem_offset);
  }
...
XCdrAccessor XCdrAccessor::operator[](size_t index)
{
...
XCdrAccessor & XCdrAccessor::operator=(std::string_view value)
{
  if (!std::holds_alternative<XCdrStringLayout>(*layout_)) {
    throw XCdrError("Type mismatch: expected string layout for string assignment");
  }

  const auto & str_layout = std::get<XCdrStringLayout>(*layout_);

  // Validate string length matches layout
  if (value.size() != str_layout.actual_length()) {
    throw XCdrError(
      "String length mismatch: expected " + std::to_string(str_layout.actual_length()) +
      ", got " + std::to_string(value.size()));
  }

  // String data starts after the length prefix
  size_t string_offset = base_offset_ + kStringLengthPrefixSize;

  if (string_offset + str_layout.actual_length() + 1 > buffer_.size()) {
    throw XCdrError("Buffer overflow writing string");
  }

  // Copy string data
  std::memcpy(buffer_.data() + string_offset, value.data(), value.size());

  // Ensure null terminator is present (should already be there from layout.apply())
  buffer_[string_offset + value.size()] = '\0';

  return *this;
}

XCdrResult<size_t> XCdrAccessor::size() const
{
  // Handle primitive arrays
  if (std::holds_alternative<XCdrPrimitiveArrayLayout>(*layout_)) {
...
XCdrAccessor::Iterator XCdrAccessor::begin()
{
  if (!std::holds_alternative<XCdrPrimitiveArrayLayout>(*layout_) &&
...
XCdrAccessor::Iterator XCdrAccessor::end()
{
...
const XCdrLayout & XCdrAccessor::layout() const
{
  return *layout_;
}
