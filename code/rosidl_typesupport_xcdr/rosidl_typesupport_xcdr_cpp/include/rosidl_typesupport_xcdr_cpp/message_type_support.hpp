...
struct rosidl_message_xcdr_cpp_type_support_t
{
  ...
  /// Populate ExternalStorage from accessor (called for construct and cast paths).
  /**
   * Walks the accessor fields and populates the ExternalStorage members
   * with MemoryRegions pointing into the buffer.
   * Called recursively for nested message members.
   * \param[in]  accessor      Opaque pointer to XCdrConstAccessor for the buffer.
   * \param[out] ext_storage   Pointer to the message's ExternalStorage struct.
   * \return RCUTILS_RET_OK on success, RCUTILS_RET_ERROR on failure.
   */
  rcutils_ret_t (*populate_external_storage)(const void *, void *) = nullptr;
