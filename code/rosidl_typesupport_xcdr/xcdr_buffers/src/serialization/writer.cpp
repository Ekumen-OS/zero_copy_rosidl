  ...
  if (!header_written_) {
    if (is_fixed_mode_) {
      if (fixed_span_.size() < kXCdrHeaderSize) {
        ...
        header_written_ = true;  // Mark as "attempted" to avoid retry loops
        return;
      }
      write_xcdr_header(tcb::span<uint8_t>(fixed_span_.data(), kXCdrHeaderSize), endianness_);
      write_position_ = kXCdrHeaderSize;
    } else {
...
void XCdrWriter::align_and_reserve(size_t alignment, size_t size)
{
  ...
  if (is_fixed_mode_) {
    // Fixed-size mode: check for overflow
...
void XCdrWriter::begin_write_sequence(size_t count)
{
  ...
  if (is_fixed_mode_) {
    ...
    write_to_bytes(length_dst, length, endianness_);
    context_stack_.push_back({write_position_, count, 0});
  } else {
...
void XCdrWriter::begin_write_struct()
{
  ensure_header_written();
  size_t pos = is_fixed_mode_ ? write_position_ : buffer_.size();
  context_stack_.push_back({pos, 0, 0});
}
